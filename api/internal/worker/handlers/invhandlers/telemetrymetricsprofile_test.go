...
func Test_TelemetryMetricsProfileHandler_Job_Error(t *testing.T) {
	...
	ctx := context.TODO()
	job := types.NewJob(ctx, BadOperation, types.TelemetryMetricsProfile, nil, inv_handlers.TelemetryMetricsProfileURLParams{})
	_, err := h.Do(job)
...
func Test_TelemetryMetricsProfileHandler_List(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
	...
	job := types.NewJob(
		context.TODO(), types.List, types.TelemetryMetricsProfile,
	...
	listResources, ok := r.Payload.Data.(api.TelemetryMetricsProfileList)
	require.True(t, ok)
	assert.NotNil(t, listResources)
	assert.Equal(t, 0, len(*listResources.TelemetryMetricsProfiles))
	...
	require.True(t, ok)
	assert.NotNil(t, listResources)
	assert.Equal(t, 3, len(*listResources.TelemetryMetricsProfiles))
...
func Test_TelemetryMetricsProfileHandler_Post(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
	...
	body := api.TelemetryMetricsProfile{
		MetricsGroupId:  telGroupRes.ResourceId,
		MetricsInterval: 300,
		TargetInstance:  &instRes.ResourceId,
	}
	job := types.NewJob(
		context.TODO(), types.Post, types.TelemetryMetricsProfile,
		&body, inv_handlers.TelemetryMetricsProfileURLParams{},
	)
	r, err := h.Do(job)
	require.Equal(t, nil, err)
	assert.Equal(t, http.StatusCreated, r.Status)
	gotRes1, ok := r.Payload.Data.(*api.TelemetryMetricsProfile)
	assert.True(t, ok)
	...
	require.Equal(t, nil, err)
	assert.Equal(t, http.StatusCreated, r.Status)
	gotRes2, ok := r.Payload.Data.(*api.TelemetryMetricsProfile)
	assert.True(t, ok)
	...
	require.Equal(t, nil, err)
	assert.Equal(t, http.StatusCreated, r.Status)
	gotRes3, ok := r.Payload.Data.(*api.TelemetryMetricsProfile)
	assert.True(t, ok)

	// Post error - wrong body request format
	job = types.NewJob(
		context.TODO(), types.Post, types.TelemetryMetricsProfile,
		&api.Host{}, inv_handlers.TelemetryMetricsProfileURLParams{},
	)
	_, err = h.Do(job)
	require.NotEqual(t, err, nil)
	assert.Equal(t, http.StatusUnprocessableEntity, errors.ErrorToHTTPStatus(err))
...
func Test_TelemetryMetricsProfileHandler_Put(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
	...
	bodyUpdate := api.TelemetryMetricsProfile{
		MetricsInterval: 300,
	...
	job := types.NewJob(
		...
		&bodyUpdate,
		inv_handlers.TelemetryMetricsProfileURLParams{TelemetryMetricsProfileID: "telemetryprofile-12345678"},
	)
	_, err := h.Do(job)
	require.Error(t, err)
	...
	job = types.NewJob(
		...
		types.TelemetryMetricsProfile,
		&bodyUpdate,
		inv_handlers.TelemetryMetricsProfileURLParams{TelemetryMetricsProfileID: profileRes.ResourceId},
	)
	r, err := h.Do(job)
...
func Test_TelemetryMetricsProfileHandler_Patch(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
...
func Test_TelemetryMetricsProfileHandler_PatchFieldMask(t *testing.T) {
	...
	expectedPatchFieldMask := []string{
		telemetryv1.TelemetryProfileFieldMetricsInterval,
...
func Test_TelemetryMetricsProfileHandler_Get(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
	...
	require.Equal(t, nil, err)
	assert.Equal(t, http.StatusOK, r.Status)
	gotRes, ok := r.Payload.Data.(*api.TelemetryMetricsProfile)
	require.True(t, ok)
...
func Test_TelemetryMetricsProfileHandler_Delete(t *testing.T) {
	client := &clients.InventoryClientHandler{
		InvClient: inv_testing.TestClients[inv_testing.APIClient],
	}
	h := handlers.NewHandlers(client, nil)
...
func Test_Inventory_TelemetryMetricsProfile_Integration(t *testing.T) {
	telemetryProfile := &telemetryv1.TelemetryProfile{}
	validateInventoryIntegration(
		t,
		telemetryProfile,
		api.TelemetryMetricsProfile{},
		inv_handlers.OpenAPITelemetryMetricsProfileToProto,
		inv_handlers.OpenAPITelemetryMetricsProfileToProtoExcluded,
		maps.Values(inv_handlers.OpenAPITelemetryMetricsProfileToProto),
		true,
	)
}
