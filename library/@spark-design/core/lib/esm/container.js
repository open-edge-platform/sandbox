...
export const containerCreator = ({ proxy, config: globalConfig }) => {
    const obj = {
        creator: null
    };
    ...
    obj.creator = (data, instanceConfig = { className: '' }) => {
        const containerConfig = globalConfig.fork(instanceConfig);
        const { className } = containerConfig.data;
        const { ...styles } = data;
        ...
        return {
            ...
            css: (options = {}) => {
                const toCssConfig = containerConfig.fork(options);
                const proxyTree = extract(data, toCssConfig);
                const { ...styles } = proxyTree;
                const containerData = { ...styles };
                const sheet = jss.createStyleSheet(containerData, {
