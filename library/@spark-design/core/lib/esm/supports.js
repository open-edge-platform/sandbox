...
export const supportsCreator = ({ proxy, config: globalConfig }) => {
    const obj = {
        creator: null
    };
    ...
    obj.creator = (data, instanceConfig = { className: '' }) => {
        const supportsConfig = globalConfig.fork(instanceConfig);
        const { className } = supportsConfig.data;
        const { ...styles } = data;
        ...
        return {
            ...
            css: (options = {}) => {
                const toCssConfig = supportsConfig.fork(options);
                const proxyTree = extract(data, toCssConfig);
                const { ...styles } = proxyTree;
                const supportsData = { ...styles };
                const sheet = jss.createStyleSheet(supportsData, {
