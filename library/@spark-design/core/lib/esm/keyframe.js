...
export const keyframeCreator = ({ proxy, config: globalConfig }) => {
    const obj = {
        creator: null
    };
    ...
    obj.creator = (data, instanceConfig = { className: '' }) => {
        const keyframeConfig = globalConfig.fork(instanceConfig);
        const { className } = keyframeConfig.data;
        const { ...styles } = data;
        ...
        return {
            ...
            css: (options = {}) => {
                const toCssConfig = keyframeConfig.fork(options);
