...
export const globalCreator = ({ proxy, config: globalConfiguration }) => {
    const obj = {
        creator: null
    };
    ...
    obj.creator = (data, instanceConfig = { className: '' }) => {
        const globalConfig = globalConfiguration.fork(instanceConfig);
        const { className } = globalConfig.data;
        const { ...styles } = data;
        ...
        return {
            ...
            css: (options = {}) => {
                const toCssConfig = globalConfig.fork(options);
