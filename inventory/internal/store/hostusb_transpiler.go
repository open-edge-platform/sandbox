...
func (r *registry) RegisterHostusbResource() {
	r.Register(
		newResourceTranspiler(
			inv_v1.ResourceKind_RESOURCE_KIND_HOSTUSB,
			hostusbresource.ValidColumn,
			map[string]edgeHandler{
				hostusbresource.EdgeHost: {
					func(p sqlPredicate) sqlPredicate { return hostusbresource.HasHostWith(p) },
					inv_v1.ResourceKind_RESOURCE_KIND_HOST,
				},
			},
			map[string]sqlPredicate{
				hostusbresource.EdgeHost: hostusbresource.HasHost(),
			},
		))
