// SPDX-FileCopyrightText: (C) 2025 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

// Code generated by protoc-gen-go-filters, DO NOT EDIT.

package store

import (
	"github.com/open-edge-platform/infra-core/inventory/v2/internal/ent/workloadresource"
	inv_v1 "github.com/open-edge-platform/infra-core/inventory/v2/pkg/api/inventory/v1"
)

func (r *registry) RegisterWorkloadResource() {
	r.Register(
		newResourceTranspiler(
			inv_v1.ResourceKind_RESOURCE_KIND_WORKLOAD,
			workloadresource.ValidColumn,
			map[string]edgeHandler{
				workloadresource.EdgeMembers: {
					func(p sqlPredicate) sqlPredicate { return workloadresource.HasMembersWith(p) },
					inv_v1.ResourceKind_RESOURCE_KIND_WORKLOAD_MEMBER,
				},
			},
			map[string]sqlPredicate{
				workloadresource.EdgeMembers: workloadresource.HasMembers(),
			},
		))

}
